...
def calculate_longitudinal_scores(panel):
    ...
    meandf = npanel.mean(axis = 'items')
    stddf = npanel.std(axis = 'items')
    maxdf = npanel.max(axis = 'items')
    mindf = npanel.min(axis = 'items')
    ...
    #use mask generated by output slope df to create new panel masking out subjects for whom the slope has already been calculated.
    negmask = np.isnan(slopedf)
    masked_values = np.where(negmask,npanel.values,np.nan)
    masked_panel = pandas.Panel(masked_values,items = npanel.items.tolist(), major_axis = npanel.major_axis.tolist(), minor_axis = npanel.minor_axis.tolist())
    
    #need to figure out a) how to remove sess_06 from the newly created masked_panel.  Also need to figure out best way to loop this function so as to not have to repeat everything again.
